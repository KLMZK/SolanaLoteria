            ...
            <div className="flex items-center gap-3">
                ...
                {connected && publicKey ? (
                    ...
                    <button
                        onClick={connect}
                        disabled={connecting}
                        className={`flex items-center gap-2 font-bold py-3 px-6 rounded-xl transition-all text-sm ${connecting
                                ? 'bg-white/10 text-white/40 cursor-wait'
                                : 'bg-[#E91E63] hover:bg-[#C2185B] active:scale-95 text-white shadow-lg shadow-pink-500/30'
                            }`}
                    >
                        {connecting ? <><span className="animate-spin">⟳</span> Conectando...</> : <>👻 Conectar Phantom</>}
                    </button>
                )}
            </div>
        </header>
